...
- First, go to the AWS Management Console and manually create a VPC, a Subnet, and a Security Group.
...
```t
resource "aws_vpc" "hepapi_vpc" {
  cidr_block = "10.0.0.0/16"
  tags = {
    Name = "terraform-import-vpc"
  }
}

resource "aws_subnet" "hepapi_subnet" {
  vpc_id     = aws_vpc.hepapi_vpc.id
  cidr_block = "10.0.1.0/24"
  tags = {
...
```
```bash
terraform import aws_vpc.hepapi_vpc vpc-0a1b2c3d4e5f6g7h8
terraform import aws_subnet.hepapi_subnet subnet-8h7g6f5e4d3c2b1a0
```
...
- version = "~> 6.1.0": The tilde and greater-than operator (~>) is a common constraint that allows for compatible minor or patch releases. For ~> 6.1.0, this means any version from 6.1.0 up to, but not including, 6.2.0 is allowed (e.g., 6.1.1, 6.1.2).
